...
- **No inbound holes** — the NAS dials _out_; nothing reaches in.
...
| 5   | **Native apps** _(future)_ — Capacitor wraps same build; native FCM/APNs branch; cloud iOS builds; stores                                                                       | later                 | iOS/Android store apps from one codebase               | ☐      |
...
- _Dev-server pushes to `main`_ → we work on a **branch**, keep `main` deployable, reconcile before cutover.
- _Don't break the live party app mid-rebuild_ → legacy PHP + flat app stay live until Phase 1 parity is proven, then retire.
- _Synology Docker port 80/443 grab_ → Caddy on alt ports + Cloudflare-later → we never fight DSM for 80/443.
