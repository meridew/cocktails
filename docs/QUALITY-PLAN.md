...
- Shared rules (validation, limits, status transitions) live in `packages/shared` and are consumed by
  *both* client and server. Never re-declare a constant that `@cocktails/shared` already exports.
...
- Comments explain *why*, not *what*. Delete stale comments; they are worse than none.
...
Chosen over Vitest: no new deps, matches the "Node built-ins" ethos, and tests run through the *same
loader that ships in Docker*. Vitest's unique wins (Svelte component tests, `import.meta.env`) are out
...
- [ ] Create `apps/api/tests/` and `apps/web/tests/` — **plural**. A dir named `test/` makes Node
  execute *every* file in it, so helper modules would run as tests ✅**ran**.
...
- [ ] **`apps/api/tests/auth.test.ts`.** Hash shape + salt randomness; round-trip; wrong password;
  ...
  email → null (assert the *outcome*, never timing — timing assertions are the top CI flake source),
...
- [ ] **`apps/api/tests/db.test.ts`.** **Migrations** (the reason for 0.1): hand-build an old-schema DB
  ...
  ⚠️ Don't assert *which* order the `maxOrders` eviction removed — all rows share a millisecond, so it's
...
- [ ] **Bound the limiter Map.** The current prune only deletes *expired* entries, so it does nothing
...
- [ ] **Bartender logout race (HIGH).** `fetchOrders` unconditionally sets `unlocked = true`; an
  in-flight poll resolving *after* `signedOut()` re-shows the queue with an empty token, so every
...
- [ ] **Mobile tap-outside-to-close is dead (MED, primary platform).** `.order-backdrop` is a *sibling*
...
- [ ] **`DELETE /api/orders/:id` returns `200 {ok:false}`** for a missing id, so the client renders
  *"Something went wrong (HTTP 200)."* and leaves the row on screen. Return `404 {ok:false,error:'not found'}`
...
- [ ] **`lib/push.svelte.ts` store — one state machine, resolved from truth.** Both `App.svelte` and
  ...
  about whether *this device+role* is registered — so the UI claims "🔔 On" while no subscription
...
- [ ] **Delete dead code** (all verified zero call sites): `api.ts` `health`; `db.ts` `getOrder`;
  ...
  legitimately client-only, so delete rather than move); the stale *"Menu (filled with data during the
  UI port)"* comment. Either use `api.ts` `me()` for session validation on mount (it's the right tool —
