...
describe('stepping a status back', () => {
  ...
  test('walking back down the chain works over HTTP', async () => {
    ...
    for (const status of ['making', 'serving', 'done']) {
      await request(`/api/orders/${order.id}`, send('PATCH', { status }, auth()));
    }
    for (const status of ['serving', 'making', 'pending']) {
      const res = await request(`/api/orders/${order.id}`, send('PATCH', { status }, auth()));
...
describe('bumping to the front', () => {
  test('a bumped order sorts ahead of older ones, newest bump first', async () => {
    ...
    const res = await request(`/api/orders/${third.id}/bump`, send('POST', {}, auth()));
    assert.equal(res.status, 200);
    assert.deepEqual(
      (await listOrders()).map((o) => o.name),
      ['Third', 'First', 'Second'],
    ...
    await request(`/api/orders/${second.id}/bump`, send('POST', {}, auth()));
    assert.deepEqual(
      (await listOrders()).map((o) => o.name),
      ['Second', 'Third', 'First'],
    );
    ...
    await request(`/api/orders/${second.id}/bump`, send('POST', { bumped: false }, auth()));
    await request(`/api/orders/${third.id}/bump`, send('POST', { bumped: false }, auth()));
    assert.deepEqual(
      (await listOrders()).map((o) => o.name),
      ['First', 'Second', 'Third'],
    );
  ...
  test('bump reports bumpedAt, and 404s for an unknown order', async () => {
    const order = await place('Dan', [{ name: 'Mojito', qty: 1 }]);
    const res = await request(`/api/orders/${order.id}/bump`, send('POST', {}, auth()));
    const { order: bumped } = (await res.json()) as { order: Order };
    assert.equal(typeof bumped.bumpedAt, 'number');

    const missing = await request('/api/orders/nope/bump', send('POST', {}, auth()));
    assert.equal(missing.status, 404);
  });

  test('is staff-only', async () => {
    const order = await place('Dan', [{ name: 'Mojito', qty: 1 }]);
    const res = await request(`/api/orders/${order.id}/bump`, send('POST', {}));
...
describe('per-drink progress', () => {
  ...
  test('completing every line reports complete', async () => {
    ...
    await request(`/api/orders/${order.id}/progress`, send('PATCH', { index: 0, made: 2 }, auth()));
    const res = await request(
      `/api/orders/${order.id}/progress`,
  ...
  test('rejects a bad index or missing fields, and 404s an unknown order', async () => {
    ...
    for (const body of [{}, { index: 'x', made: 1 }, { index: -1, made: 1 }, { index: 0 }]) {
      const res = await request(`/api/orders/${order.id}/progress`, send('PATCH', body, auth()));
