...
import { JOIN_CODE_LENGTH, JOIN_CODE_TTL_MS, type Staff } from '@cocktails/shared';
import { app } from '../src/app.ts';
import { hashPassword } from '../src/auth.ts';
import { createStaff, genId, clearJoinCodes, listStaff, staffForDevice } from '../src/db.ts';
...
async function mintCode(): Promise<{ code: string; expiresAt: number }> {
  const res = await app.request('/api/staff/join-code', send('POST', {}, asAdmin()));
...
const redeem = (code: string, name: string, deviceId: string, ip = freshIp()) =>
  app.request(
    '/api/staff/join',
    send('POST', { code, name, deviceId }, { 'cf-connecting-ip': ip }),
  );
...
before(async () => {
  ...
  adminToken = (
    (await (await app.request('/api/auth/login', send('POST', ADMIN))).json()) as {
      token: string;
    }
  ).token;
  helperToken = (
    (await (await app.request('/api/auth/login', send('POST', HELPER))).json()) as {
      token: string;
    }
  ).token;
...
describe('minting a code', () => {
  ...
  test('is closed to strangers', async () => {
    assert.equal((await app.request('/api/staff/join-code', send('POST', {}))).status, 401);
  });
});
...
describe('redeeming a code', () => {
  ...
  test('the session it issues really works, but is not an admin one', async () => {
    ...
    assert.equal((await app.request('/api/orders', { headers: auth })).status, 200);
    // A code must never be an escalation path to approving other people.
    assert.equal((await app.request('/api/staff', { headers: auth })).status, 403);
  });
  ...
  test('a device that already asked is upgraded, not duplicated', async () => {
    ...
    await app.request('/api/staff/requests', send('POST', { name: 'Mo', deviceId: 'device-mo' }));
  ...
  test('a revoked code stops working immediately', async () => {
    ...
    const res = await app.request('/api/staff/join-code', {
      method: 'DELETE',
      headers: asAdmin(),
    });
    assert.equal(res.status, 200);
  ...
  test('revoking codes does not evict helpers who already joined', async () => {
    ...
    await app.request('/api/staff/join-code', { method: 'DELETE', headers: asAdmin() });
