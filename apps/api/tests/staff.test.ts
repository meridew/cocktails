...
import type { Staff, StaffClaimResponse } from '@cocktails/shared';
import { app } from '../src/app.ts';
import { hashPassword } from '../src/auth.ts';
import { createStaff, genId, listStaff, deleteStaff } from '../src/db.ts';
...
async function askToHelp(name: string, deviceId: string): Promise<string> {
  const res = await app.request('/api/staff/requests', post({ name, deviceId }));
...
const claim = async (secret: string): Promise<StaffClaimResponse> => {
  const res = await app.request('/api/staff/claim', post({ claim: secret }));
...
before(async () => {
  ...
  const res = await app.request('/api/auth/login', post(ADMIN));
...
describe('asking to help', () => {
  test('requires a name and a device', async () => {
    for (const body of [
      ...
      const res = await app.request('/api/staff/requests', post(body));
  ...
  test('creates a pending member the admin can see, and grants nothing yet', async () => {
    ...
    const listed = await app.request('/api/staff', { headers: auth(adminToken) });
...
describe('approval', () => {
  ...
  test('the session works on staff routes but not on admin ones', async () => {
    ...
    const sarah = pendingFor('Sarah');
    assert.ok(sarah);
    await app.request(`/api/staff/${sarah.id}/approve`, post({}, auth(adminToken)));
    const collected = await claim(secret);
    assert.equal(collected.status, 'active');
    const helperToken = collected.status === 'active' ? collected.token : '';

    const queue = await app.request('/api/orders', { headers: auth(helperToken) });
    assert.equal(queue.status, 200, 'a helper can run the bar');

    // 403 not 401: authenticated, but not permitted — and can't self-promote.
    const list = await app.request('/api/staff', { headers: auth(helperToken) });
  ...
  test('approving something that is not pending fails', async () => {
    const res = await app.request('/api/staff/nope/approve', post({}, auth(adminToken)));
  ...
  test('only an admin can approve — anonymous callers get 401', async () => {
    ...
    const sarah = pendingFor('Sarah');
    assert.ok(sarah);
    const res = await app.request(`/api/staff/${sarah.id}/approve`, post({}));
  ...
  test('denying removes the request, and its claim stops working', async () => {
    ...
    const denied = await app.request(`/api/staff/${mallory.id}`, {
      method: 'DELETE',
...
describe('revocation', () => {
  ...
  async function approvedHelper(name: string, deviceId: string) {
    ...
    assert.ok(row);
    await app.request(`/api/staff/${row.id}/approve`, post({}, auth(adminToken)));
    const collected = await claim(secret);
    assert.equal(collected.status, 'active');
  ...
  test('revoking takes effect immediately, despite an unexpired token', async () => {
    ...
    assert.equal((await app.request('/api/orders', { headers: auth(helper.token) })).status, 200);

    const revoked = await app.request(`/api/staff/${helper.id}/revoke`, post({}, auth(adminToken)));
    assert.equal(revoked.status, 200);

    const after = await app.request('/api/orders', { headers: auth(helper.token) });
  ...
  test('revoke-all clears every helper but leaves the admin signed in', async () => {
    ...
    const res = await app.request('/api/staff/revoke-all', post({}, auth(adminToken)));
    assert.equal(res.status, 200);

    for (const helper of [a, b]) {
      assert.equal((await app.request('/api/orders', { headers: auth(helper.token) })).status, 401);
    }
    const stillAdmin = await app.request('/api/auth/me', { headers: auth(adminToken) });
  ...
  test('an admin cannot be revoked or removed (no lock-out)', async () => {
    const me = await app.request('/api/auth/me', { headers: auth(adminToken) });
    const { staff } = (await me.json()) as { staff: Staff };

    const revoke = await app.request(`/api/staff/${staff.id}/revoke`, post({}, auth(adminToken)));
    assert.equal(revoke.status, 403);

    const remove = await app.request(`/api/staff/${staff.id}`, {
      method: 'DELETE',
      headers: auth(adminToken),
    });
    assert.equal(remove.status, 403);

    const after = await app.request('/api/auth/me', { headers: auth(adminToken) });
  ...
  test('a revoked helper cannot sign in, and asking again starts a fresh request', async () => {
    const helper = await approvedHelper('Sarah', 'dev-sarah');
    await app.request(`/api/staff/${helper.id}/revoke`, post({}, auth(adminToken)));
