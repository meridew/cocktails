...
import { PIN_LENGTH, isValidPin, type Staff } from '@cocktails/shared';

/**
 * `config` freezes the PIN when it is first imported, and static imports are
 * hoisted above any code in this file — so nothing under `../src/` may be imported
 * statically here, or the env below would be set too late to matter. (The pure
...
let seedStaff: typeof import('../src/auth.ts').seedStaff;
...
const tryPin = (pin: string, ip = freshIp()) => app.request('/api/auth/pin', from(ip, { pin }));
...
before(async () => {
  ...
  ({ seedStaff } = await import('../src/auth.ts'));
...
describe('POST /api/auth/pin', () => {
  ...
  test('the session it issues really works', async () => {
    ...
    const staff = await app.request('/api/staff', {
  ...
  test('malformed input is refused without leaking how it failed', async () => {
    ...
    for (const pin of ['', '1', '12345', '1234567', 'abcdef', '12 456']) {
      const res = await app.request('/api/auth/pin', from(ip, { pin }));
